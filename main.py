...
print(f"[BOT] WEBAPP_URL={WEBAPP_URL} PORT={PORT}")
...
if not DATABASE_URL:
    logger.error("DATABASE_URL is not set. Please add PostgreSQL in Railway.")
    exit(1)
        ...
        with conn.cursor() as cur:
            cur.execute("""
                CREATE TABLE IF NOT EXISTS orders (
                    ...
                    fio TEXT NOT NULL,
                    phone TEXT NOT NULL,
                    status TEXT NOT NULL DEFAULT 'в модерации',
                    photo_id TEXT,
            ...
            """)
            conn.commit()
            logger.info("Database table 'orders' is ready.")
    finally:
        conn.close()

init_db()

def db_create_order(user_id, fio, phone, referred_by=None):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO orders (user_id, fio, phone, status, referred_by) VALUES (%s, %s, %s, %s, %s) RETURNING id",
                (user_id, fio, phone, 'в модерации', referred_by)
            )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    poster_url = f"{WEBAPP_URL}mini_app/legenda_bg.png"
...
async def renotify_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if order.get('photo_id'):
        is_pdf = str(order.get('photo_id', '')).startswith('BQ')
        text = (
...
@flask_app.route("/api/shop/stats")
def api_stats():
    total = 2000
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM orders WHERE status = 'заказ принят'")
            confirmed = cur.fetchone()[0]
    finally:
        conn.close()
    remaining = max(0, total - confirmed)
    return jsonify({"confirmed": confirmed, "remaining": remaining, "total": total})

@flask_app.route("/api/shop/orders")
def api_orders():
    conn = get_db()
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT id, fio, phone, status, is_bonus FROM orders ORDER BY id")
            rows = cur.fetchall()
    finally:
        conn.close()
    return jsonify([{
...
@flask_app.route("/api/shop/my-orders")
def api_my_orders():
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            if user_id:
                cur.execute("SELECT * FROM orders WHERE user_id = %s ORDER BY created_at DESC", (int(user_id),))
            else:
                digits = ''.join(c for c in phone if c.isdigit())
                cur.execute(
                    "SELECT * FROM orders WHERE regexp_replace(phone, '[^0-9]', '', 'g') LIKE %s ORDER BY created_at DESC",
                    (f'%{digits[-10:]}%',)
                )
            rows = cur.fetchall()
    finally:
...
@flask_app.route("/api/shop/my-order")
def api_my_order():
    user_id = request.args.get("user_id")
    if not user_id:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id FROM orders WHERE user_id = %s AND NOT is_bonus ORDER BY created_at DESC LIMIT 1",
                (int(user_id),)
            )
            row = cur.fetchone()
    finally:
        conn.close()
    return jsonify({"order_id": row[0] if row else None})

@flask_app.route("/api/shop/referrals")
def api_referrals():
    order_id = request.args.get("order_id")
    if not order_id:
        return jsonify({"progress": 0, "needed": 10, "bonuses": 0})
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM orders WHERE referred_by = %s AND status = 'заказ принят' AND NOT is_bonus", (int(order_id),))
            confirmed = cur.fetchone()[0]
            cur.execute("SELECT COUNT(*) FROM orders WHERE referred_by = %s AND is_bonus", (int(order_id),))
            bonuses = cur.fetchone()[0]
    finally:
        conn.close()
    return jsonify({
        "progress": confirmed,
...
@flask_app.route("/api/shop/orders", methods=["POST"])
def api_create_order():
    data = request.get_json()
    if not data or "fio" not in data or "phone" not in data:
        return jsonify({"error": "Missing fields"}), 400
    user_id = data.get("user_id", 0)
    referred_by = data.get("referred_by")
    order_id = db_create_order(user_id, data["fio"], data["phone"], referred_by)
